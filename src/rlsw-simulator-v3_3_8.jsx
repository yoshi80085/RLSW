...
const SPIRIT_DEFS = {
  "cosmic_ronin":      { id:"cosmic_ronin",      name:"Cosmic Ronin",      imageSrc:cosmic_ronin,      color:"#4488ff", vibe:5, maxVibe:5, knockedOut:false, style:"Shred",  drive:8, sustain:5, speed:5 },
  ...
  "Metalness_Monster": { id:"Metalness_Monster", name:"Metalness Monster", imageSrc:metalness_monster, imageOffset:{ x:20, y:-30 }, color:"#ffcc00", vibe:5, maxVibe:5, knockedOut:false, style:"Shred",  drive:7, sustain:6, speed:4 },
  ...
  return (
    <div style={{display:"flex", flexDirection:"column", gap:14}}>
      ...
      <div style={{display:"flex", gap:8}}>
        <MockSpiritCard name="Cosmic Ronin" style="Shred" drive={8} sustain={5} vibe={5} maxVibe={5} color="#4488ff"/>
...
function TutSection_DriveSustain() {
  return (
    <div style={{display:"flex", flexDirection:"column", gap:14}}>
      <div style={{display:"flex", gap:8}}>
        <MockSpiritCard name="Cosmic Ronin" style="Shred" drive={8} sustain={5} vibe={4} maxVibe={5} color="#4488ff" tempDrive={2}/>
...
function TutSection_Spirits() {
  const spirits = [
    { id:"cosmic_ronin",      name:"Cosmic Ronin",      style:"Shred",  drive:8, sustain:5, vibe:5, maxVibe:5, color:"#4488ff",
...
function Game({ gameState, onReturnToLobby }) {
  ...
  function confirmNoteTrack() {
    ...
    addLog(`✓ Committed · ${hexes} hexes${scoreStr}${driveMsg}${sustMsg}${triMsg}${octMsg}${majorThirdMsg}${m7Msg}${tritoneEndMsg}${chrMsg}${chromClimbMsg}${feedbackOverloadMsg}${speedMsg} · Next RN: ${newRootRaw} (pick Major/Minor)`);
  ...
  function grantFame(spiritId, fp, reason) {
    if (fp <= 0) return;
    const sp = spirits.find(s => s.id === spiritId);
    const newFame = (noteStates[spiritId]?.fame ?? 0) + fp;
    setNoteStates(prev => ({
      ...prev,
      [spiritId]: { ...prev[spiritId], fame: (prev[spiritId]?.fame ?? 0) + fp },
    }));
    addLog(`⭐ ${sp?.name} earns ${fp} Fame Point${fp !== 1 ? 's' : ''}${reason ? ` — ${reason}` : ''}! (${Math.min(newFame, FAME_TO_WIN)}/${FAME_TO_WIN})`);
  ...
  function applyVibeDamage(targetId, dmg, sourceLabel) {
    setSpirits(prev => prev.map(s => {
  ...
  function initiateSonicAttack(targetId) {
    ...
    const atkRoll  = Math.floor(Math.random() * dieSides) + 1;
    const rawDefRoll = defenderPosing ? 0 : randD6();
    const defRoll  = defenderPosing ? 0
      : skillMods.halveDef ? Math.max(1, Math.floor(rawDefRoll / 2)) : rawDefRoll;
    const atkTotal = atkStat + atkRoll;
  ...
  function riffNextNote(turn, idx) {
    ...
    riffEngineRef.current = { turn, idx, shownAt: performance.now(), resolved: false, timeoutId: null, window: win };
    setBattleState(p => p?.riffOff ? { ...p, phase: 'riff_play', turn, noteIdx: idx } : p);
  ...
  function closeRiffOff() {
    ...
    if (!tie) {
      ...
      applyVibeDamage(loserId, damage, spirits.find(x => x.id === winnerId)?.name);
  ...
  function closeBattleOverlay() {
    ...
    if (attackerWon) {
      if (margin <= 2) {
        ...
        applyVibeDamage(defenderId, damage, spirits.find(s2 => s2.id === attackerId)?.name);
      ...
      applyVibeDamage(attackerId, selfDmg, 'whiff');
  ...
  useEffect(() => {
    ...
    if (retaliationTimer <= 0) {
      ...
      if (bs) {
        ...
        applyVibeDamage(bs.defenderId, bs.damage, 'swing');
  ...
  function finishCounter() {
    ...
    if (bs.counterSuccess) {
      ...
      applyVibeDamage(bs.attackerId, counterDmg, 'counter');
      ...
      if (bs.pendingPush) battleKnockback(bs.attackerId, bs.defenderId, knockbackSpaces(bs));
      applyVibeDamage(bs.defenderId, extraDmg, 'failed_counter');
            ...
            <div style={{
              ...
              {phase === 'riff_play' && (
                <div style={cardBase(noteColor)}>
                  ...
                  <div key={`${rTurn}-${rNoteIdx}`} style={{fontSize:80, fontWeight:900, color:'#fff',
                    textShadow:`0 0 34px ${noteColor}`, animation:'riffpulse 0.18s ease-out',
                    lineHeight:1.1, margin:'4px 0 0'}}>
                    {curNote ? noteGlyph(curNote) : ''}
                  </div>
                  <div style={{fontSize:9, letterSpacing:2, height:14,
                    color: curSharp ? '#ffcc44' : '#1f2f4a'}}>
                    {curSharp ? '⬆ SHARP — HOLD SHIFT' : '​'}
                  </div>
                  {/* Window timer — rushed notes burn hot and fast */}
                  {(() => {
                    const beat   = rRiff?.rhythm?.[rNoteIdx] ?? {};
                    const curWin = beat.window ?? RIFF_NOTE_WINDOW;
                    const barCol = beat.feel === 'rushed' ? '#ff6633' : noteColor;
                    return (
