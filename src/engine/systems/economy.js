...
export function applyNoteStatesSynced(state, { noteStates }) {
  // 🪦 The `peakCasuals` high-water merge is GONE (R5, 2026-09-15). It existed so
  // a crowd you once had could never stop paying for Sonic die size, and it had to
  // survive a client sync to do that. With `sonicDieSides` deleted there is no
  // ratchet left to protect — casuals are simply a live count again, and they pay
  // through `FAN_CASUAL_WEIGHT` like Diehards do.
  return { ...state, noteStates:{ ...noteStates } };
}
