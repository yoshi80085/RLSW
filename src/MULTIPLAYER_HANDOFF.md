...
| 7. Bot policies | ◑ **7a DONE (2026-07-08):** pure scorer functions + constants extracted to `engine/policies/bot.js`: `BOT_PERSONALITIES`, `BOT_PERSONA_KEYS`, `BOT_SKILL_PRIORITY_BASE`, `BOT_SPIRIT_SKILLS`, `SPIRIT_ONLY_ROUTE`, `BOT_RIFF_PROFILE` (constants); `botAssignPersona`, `botPickTarget`, `botHexScore`, `botSkillEligible`, `botPickSkillTarget`, `botRiffResults` (pure functions). Main file has thin wrappers that pass closure state (engineRef, SKILL_BY_ID). `botRiffResults` now threaded through `RANDOM_BATCH_DRAWN` engine rng (replay-deterministic). `botPersona` assignment uses `botAssignPersona` + engine rng (was `Math.random()`). selftest: persona assignment, target picking, hex scoring, skill eligibility, skill target picking, riff results determinism + shape. **Remaining:** 7b plan functions → `policy(state, rng)` returning engine actions; 7c step-machine slim. |
