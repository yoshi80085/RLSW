...
cool down normally.**
**Remaining Phase 6:** event resolution (trivia dice → engine rng; `resolveActiveEvent`
d6 + Disco Inferno hex picks → engine rng). Boss cinematics stay client.
...
| 6. Events / FX / Rock God | ◑ **6b + 6c COMPLETE; 6d COMPLETE; 6a BOARD ACTIONS COMPLETE.** Data-layer rng prep: all `Math.random()` in `data/` injectable. **6b COMPLETE:** stage FX fully engine-owned (deck/thresholds/active effects/ticks). **6c COMPLETE:** Rock God fully engine-owned (summon/damage/act/defeat/triumph/timer). **6d COMPLETE (2026-07-08):** debuffs + burn ticks (engine rules), PLUS spotlight heal/move, token scatter, flaming decay, event respawn/spawn, charge zone ticks — all semantic engine actions. **6a BOARD ACTIONS COMPLETE (2026-07-08):** `SPOTLIGHT_HEALED/MOVED`, `TOKENS_SCATTERED`, `TOKEN_PICKED_UP`, `FLAMING_HEXES_SET/DECAYED`, `EVENT_HEX_TRIGGERED`, `EVENT_RESPAWN_TICKED`, `EVENT_HEX_SPAWNED`, `CHARGE_ZONE_USED/TICKED`. All 6 board compat shims deleted (zero callers); `BOARD_SYNCED` bridge removed; `spawnEventHex` deleted. ⚠️ spotlight move + token scatter + event spawn now seeded rng (were `Math.random()`). selftest: all board actions + replay proof (full-log + snapshot/restore/tail + JSON-safe). **Remaining:** event resolution rng (trivia d6 + Disco Inferno hex picks → engine rng). Boss cinematics stay client. |
| 7. Bot policies | ☐ pre-analyzed plan ready — §5e. Blocked until 5+6 land (bots must read engine state). |
| 8. Serialize + replay | ◑ **PARTIAL PROOF LANDED** (8c over today's engine-owned systems). selftest replays a scripted multi-system log (turn → move → attack → counter → damage → knockdown → winner → riff-off) and asserts `snapshot(replay(restore(snapshot(s0)), log)) === snapshot(live)` **byte-for-byte**, plus mid-game snapshot/restore/replay-tail and same-seed determinism. Widens to note-track/skill/event/god actions as those land (post-flip). **8b DONE** — full-state audit clean (nothing non-JSON stored) + `assertJsonSafe()` guard added to `serialize.js` and enforced in selftest (walks real + replayed state; asserts it bites on Set/Infinity/NaN/undefined/Date/function). **8a DONE (2026-07-07):** `dispatch` records `{action, cursorBefore}`; Testing Grounds → 💾 EXPORT ACTION LOG downloads `{schema, seed, config, actionCount, log}` (the full server-replay bundle). Replay proof widened over FAME_CHANGED/FANS_CHANGED/NOTE_SHEET_PATCHED/FANS_TICKED. **2026-07-08:** widened again — the 6b active-FX lifecycle and the 6c boss lifecycle each replay byte-identically in selftest (activation/ticks and summon/act/damage/defeat, all on the logged rng stream). **2026-07-08 (later):** widened again — DEBUFFS_TICKED + BURN_TICKED (6d) replay byte-identically (full-log + snapshot/restore/tail + JSON-safe). **2026-07-08 (board actions):** widened again — all 12 board actions (SPOTLIGHT_HEALED/MOVED, TOKENS_SCATTERED, TOKEN_PICKED_UP, EVENT_HEX_TRIGGERED, EVENT_RESPAWN_TICKED, EVENT_HEX_SPAWNED, CHARGE_ZONE_USED, CHARGE_ZONES_TICKED, FLAMING_HEXES_SET, FLAMING_DECAYED) replay byte-identically (full-log + snapshot/restore/tail + JSON-safe). Remaining: 8c full-system widening as remaining 6a/7 land + 8d divergence hunt (pre-analyzed §5f). Owner: `npm run test:engine` + commit from Windows. |
