...
6b stage FX — ☑ **COMPLETE (2026-07-08), pending smoke-test.** Deck +
thresholds (2026-07-07): `state.stageFx { deck, fired }` — deck shuffled ONCE
at `makeInitialState` on a `"stageFxDeck"` fork (⚠️ behavior delta: show order
SEEDED per game); `STAGE_FX_DRAWN` exactly-once per threshold. **ACTIVE
EFFECTS + TICKS (2026-07-08):** `smoke/laser/pyro/animatronics` moved into
`state.stageFx`; `STAGE_FX_ACTIVATED { fxId, occupied }` creates the effect
(beam patterns / pyro hexes / animatronic spawns on ENGINE rng — `board/
stageFx.js` rollers now take injectable `rand`, and `spawnAnimatronics` a
`keyBase` so engine keys are deterministic, not Date.now); `STAGE_FX_TURN_
TICKED` (pyro arm→erupt→re-arm→burnout + animatronic step/slam/expiry) and
`STAGE_FX_ROUND_TICKED` (smoke spread/clear, laser re-pattern/off) are engine
rules dispatched at the same end-of-turn/round beats. Reports (`lastActivation/
lastTurnTick/lastRoundTick` incl. zapped/caught/hit spirit ids, computed from
ENGINE positions) drive the client logs/FX/damage — damage still flows through
`applyVibeDamage` → `DAMAGE_APPLIED`. `useStageEffects` is banner-only; the
`stageFxHazardRef` mirror is DELETED (`checkStageFxHex` reads `engineRef
.current.stageFx`; divineShield read moved to `engineRef` noteStates — strictly
fresher). ⚠️ behavior deltas (intended): all pattern/spawn/tie-break rolls are
seeded now; laser render `key` derives from roundsLeft (was Date.now).
selftest: full lifecycle per effect (activation shape, spread/clear, re-pattern
/off, erupt-with-caught, re-arm-excludes-previous, burnout, spawn/step/expiry,
deterministic keys, planted-spirit zap/caught reports, idle no-op), byte
replay, JSON-safe.
6c Rock God — ☑ **COMPLETE (2026-07-08), pending smoke-test.** (Attack pick was
engine-owned 2026-07-07.) **STATE OWNERSHIP:** `state.rockGod { summoned, god
{ id num hp maxHp winded telegraph lastAttack }, outcome, lastPick, lastHit,
lastAct, lastTimerExpiry }`. Actions: `GOD_SUMMONED { leaderId, godId }` (pick
stays client — `pickRockGod` reads amps; HP scales off ENGINE living-spirit
count; dedup = one god per game), `GOD_DAMAGED { spiritId, dmg }` (RAW damage
in, engine owns the winded ×2 + HP floor + kill-blow telegraph clear; report
`lastHit`), `GOD_ACTED` (the WHOLE end-of-turn answer: telegraph resolve /
winded recovery / weighted engine-rng open — slide aims at the engine FP
leader, mosh shoves MOVE the engine spirits, boxed-in crushed ids reported;
report `lastAct`), `GOD_DEFEATED`/`GOD_TRIUMPHED` (outcome lock, replaces
`bossOutcome`), `GOD_TIMER_EXPIRED { spiritId }` (the seam: countdown stays
client, expiry is replay-logged; vengeance damage rides `DAMAGE_APPLIED`).
Client: `rockGod`/`bossOutcome` are VIEWS of the engine slice; `rockGodRef` +
`godSummonedRef` mirrors DELETED (async reads → `engineRef.current.rockGod`);
`useRockGod` keeps only the clock + banner; `rockGodAct`/`attackRockGod`/
`summonRockGod` are dispatch + report renderers; the two non-combat `setWinner`
sites (champion crowning, Fame-runaway) now shadow `WINNER_DECLARED` — the
remaining-cleanup note from the spirit flip is CLOSED. ⚠️ minor test-lever
delta: 🧪 HURT GOD now routes through `GOD_DAMAGED` (doubles while winded).
selftest: summon shape/HP-scaling/dedup, raw + winded-doubled + kill-blow
damage, thunderclap + power-slide resolve (caught from engine positions, god
moves + winded), recovery beat, all four open buckets via pinned rands
(telegraph AoE armed, slide targets FP leader, melt nearest, mosh moves engine
spirits + shove/crush partition), outcome locks + no-flip, timer record,
no-act-after-outcome, full-lifecycle byte replay, JSON-safe.
6d end-of-turn ticks — ◑ **PARTIAL (2026-07-08): debuffs + burn are engine
rules, pending smoke-test.** `DEBUFFS_TICKED { spiritId }` clears tripped/dazed/
instrumentDropped, decrements mojoDrain (−1, floor 0), ticks stagger.turnsLeft
(expiry clears to null); pure, no rng; report `turn.lastDebuffTick { spiritId,
cleared, tripped, dazed, instrumentDropped, mojoDrainBefore, staggerBefore }`.
`BURN_TICKED { spiritId }` flips a 50/50 on ENGINE rng: < 0.5 → subtract 1 Vibe
from engine spirits (floored at 0, same as DAMAGE_APPLIED); always decrements
burn.turnsLeft (expiry clears burn to null); report `turn.lastBurnTick
{ spiritId, burnDamage, turnsLeft, expired }`; client reads the report for
log/FX and dispatches KNOCKDOWN_RESOLVED after its 80ms cinematic delay if Vibe
hit 0 (same pattern as combat). ⚠️ **BEHAVIOR DELTA (intended):** the burn
50/50 coin is NOW seeded rng (was `Math.random()`), so burn outcomes are
replay-deterministic. selftest: debuff clear/decrement/expire/no-op/unknown-id;
burn coin-damage/no-damage, expiry, vibe floor, determinism, byte-identical
replay; full-log + snapshot/restore/tail + JSON-safe.
☑ **Remaining 6d DONE (2026-07-08):** spotlight heal, spotlight move, board token
scatter, disco inferno decay, event respawn, charge zone cooldown — all migrated
to semantic engine actions (`SPOTLIGHT_HEALED`, `SPOTLIGHT_MOVED`,
`TOKENS_SCATTERED`, `FLAMING_DECAYED`, `EVENT_RESPAWN_TICKED` + `EVENT_HEX_SPAWNED`,
`CHARGE_ZONES_TICKED`). Board pickup/trigger sites also migrated:
`EVENT_HEX_TRIGGERED`, `TOKEN_PICKED_UP`, `CHARGE_ZONE_USED`, `FLAMING_HEXES_SET`.
All 6 compat shims (`setSpotlightHex`, `setEventHexes`, `setEventRespawnIn`,
`setFlamingHexes`, `setBoardTokens`, `setChargeZones`) deleted — zero callers remain.
`BOARD_SYNCED` bridge import removed (dead). `spawnEventHex` function deleted (dead).
Dead imports pruned (`makeBoardToken`, `EVENT_HEX_POOL`). ⚠️ **BEHAVIOR DELTA
(intended):** spotlight move + token scatter are now engine-rng (were `Math.random()`);
event hex spawns are engine-rng. Board layout decisions are replay-deterministic.
selftest: all board actions + replay proof (full-log + snapshot/restore/tail +
JSON-safe). **⚠️ Owner: `npm run dev` — play through a full round to confirm spotlight
moves, tokens scatter, disco inferno decays, event respawn fires, and charge zones
cool down normally.**
☑ **Remaining Phase 6 DONE (2026-07-08):** event resolution rng migrated via
`RANDOM_BATCH_DRAWN` — a generic "draw N raw [0,1) engine rng values" action.
`resolveActiveEvent` pre-draws all dice/picks it needs (d6 closures, Disco Inferno
hex picks, séance_27 stagger shuffle), `checkEventTrigger` pre-draws 2 values for
trivia question pick + bot trivia odds. All `Math.random()` in event resolution is
now engine-sourced and in the action log → replay-deterministic. Client code stays
unchanged (same event modal, same logs/FX). Boss cinematics stay client.
selftest: draw count/cursor, determinism, d6 range, chained draws, replay proof.
...
| 6. Events / FX / Rock God | ☑ **COMPLETE (2026-07-08).** Data-layer rng prep: all `Math.random()` in `data/` injectable. **6b COMPLETE:** stage FX fully engine-owned. **6c COMPLETE:** Rock God fully engine-owned. **6d COMPLETE:** debuffs/burn/spotlight/tokens/flaming/event-respawn/charge — all semantic engine actions. **6a BOARD ACTIONS COMPLETE:** all 12 board actions engine-owned; 6 compat shims deleted. **Event resolution rng COMPLETE:** `RANDOM_BATCH_DRAWN` action (generic N-value engine rng draw); `resolveActiveEvent` + `checkEventTrigger`/`pickTrivia`/bot trivia odds all use pre-drawn engine rng values instead of `Math.random()`. selftest: draw semantics + replay proof. |
| 7. Bot policies | ☑ **COMPLETE (2026-07-09). 7a DONE (2026-07-08):** pure scorer functions + constants extracted to `engine/policies/bot.js`: `BOT_PERSONALITIES`, `BOT_PERSONA_KEYS`, `BOT_SKILL_PRIORITY_BASE`, `BOT_SPIRIT_SKILLS`, `SPIRIT_ONLY_ROUTE`, `BOT_RIFF_PROFILE` (constants); `botAssignPersona`, `botPickTarget`, `botHexScore`, `botSkillEligible`, `botPickSkillTarget`, `botRiffResults` (pure functions). Main file has thin wrappers that pass closure state (engineRef, SKILL_BY_ID). `botRiffResults` now threaded through `RANDOM_BATCH_DRAWN` engine rng (replay-deterministic). `botPersona` assignment uses `botAssignPersona` + engine rng (was `Math.random()`). selftest: persona assignment, target picking, hex scoring, skill eligibility, skill target picking, riff results determinism + shape. **7b DONE (2026-07-08):** bot plan functions extracted as pure policies: `botPlanNoteStep` (note-track planner), `botSpiritChord` (chord eval w/ intergalactic_0 override), `botPlanRevoice` (chord revoice planner), `botRivalsWithin` (distance query), `botPlanMove` (movement planner — takes `amps` param, amps not yet engine-owned). Private `botIsNotePlayable` rebuilt purely from noteState fields + `DISCORD_INTERVAL_MAP` constant (no closure dep on Game's `isNotePlayable`). Main file: thin wrappers passing closure state (engineRef, amps). `botBestFacing`/`botNeighborForAmp` left in main file (deep closure deps on `getRivalsInBeam`/`getRivalsInCone`/`ampPlaceCandidates`). selftest: note step slot validity, musical persona tritone avoidance, track commit, spirit chord drive/sustain/intergalactic override, revoice null cases, rivals KO exclusion, move hex-or-null. **7c DONE (2026-07-09):** `botBusyRef` deleted — debounce folded into `botStepRef` via a `'pending'` state. `schedule()` sets `botStepRef.current = 'pending'`; the step-machine guard checks `step === 'pending'` instead of the old ref. When the timeout fires, if `fn()` didn't advance the step itself the previous step is restored (so empty beats and guard-wrapped actions all work). Turn-change reset + watchdog cleanup simplified (just `botStepRef.current = 'idle'`). No code references to `botBusyRef` remain. **Determinism proof:** selftest simulates a full build-phase decision loop (note selection → commit) for all 4 personas and asserts identical action sequences on identical state; plus movement, revoice, target-picking, and riff-results determinism. esbuild-compile clean (replay-onto-HEAD). selftest green. |
| 8. Serialize + replay | ☑ **COMPLETE (2026-07-09).** 8a (action log + export), 8b (JSON-safe audit), and **8c COMPREHENSIVE replay proof** all green. The 8c test is one continuous ~50-action scripted log spanning EVERY engine system in a single sequence: `gameInit` → `spiritsSynced` → turn/move/facing → combat (swing + counter + damage + knockdown) → economy (fame/fans/noteSheet) → board (spotlight/tokens/events/charge/flaming) → stage FX (draw + activate + turn/round ticks) → Rock God (summon + damage + act) → debuffs + burn → riff-off (full 2-round w/ round-2 sudden-death) → fan ticks → random batch → winner → turn end. Asserts: (1) replay from serialized start == live play byte-for-byte, (2) mid-game snapshot/restore/tail at 4 cut points, (3) same-seed determinism from scratch, (4) JSON-safe final state, (5) ≥30 distinct action types covered. 8d (divergence hunt) not needed — no divergence found. **EXIT CRITERION MET: a server replaying the log gets the same game.** |
