...
**Remaining 6d:** spotlight heal, spotlight move, board token scatter, disco
inferno decay, event respawn, charge zone cooldown — all blocked on their React
state (`spotlightHex`, `flamingHexes`, `boardTokens`, `eventHexes`,
`eventRespawnIn`, `chargeZones`) joining engine state. That migration overlaps
heavily with 6a (event/card/charge picks need the same state ownership), so
**bundle 6a + remaining 6d** in one session.
**Remaining Phase 6:** 6a (events/cards/charge picks) + remaining 6d (round-end
ticks). Boss cinematics stay client.
