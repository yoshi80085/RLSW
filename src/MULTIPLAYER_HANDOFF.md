...
| 6. Events / FX / Rock God | ◑ **STARTED — 6c prep (Rock God rng determinism).** `pickGodAttack`/`godTauntLine` in `data/rockGods.js` now take an injectable `rand = Math.random` (default preserves live behavior — dormant, same treatment as Phase-4 riffGeneration / Phase-5a economy); the seeded engine rng gets threaded when the `GOD_ATTACKED` action lands at 6c. selftest locks the weighted-draw boundaries (cum. weight 3/6/8/10), the no-immediate-repeat rule, single-attack + empty-deck edge cases, taunt indexing, determinism, and `pickRockGod` fallback. Verified green against the real modules (VM up). Rest of §5d (6a events/cards/charge, 6b stage FX, 6c actions + timer seam, 6d END-TURN tick order) still pre-analyzed. |
