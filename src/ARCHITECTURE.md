...
| `gameConstants.js` | 570 | 107 named constants — `FAME_TO_WIN`, `LIMELIGHT_TO_WIN`, `POSE_FP_STEP`, `RIG_RADIUS_FLOOR`, `RIG_ATROPHY_TURNS`, `FAN_*`, `SMASH_*`, `THRASH_*`, `SONIC_*`, `TOKEN_*`, `TOKEN_UNLOCK_SPAWN_SHARE`, `STOCK_PALETTE_GUARANTEE`, `stackCapFor`, … | **All gameplay tuning.** ⚠️ `stackCapFor(noteSheet, which)` changed shape 2026-09-02 — it reads `driveSlots`/`sustainSlots` per stack and THROWS on the old `unlockedSkills` array, because the quiet version of that failure caps every stack at 3 forever. |
