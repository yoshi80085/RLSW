// A/B for §6.6.8 — the pose, before and after. Runs UNCHANGED on both trees:
// HEAD has no `limelightScores` on the result and no `pose` payout at all, so
// those columns simply read 0 there. That is the finding, not a gap in the probe.
...
let turns = 0, dec = 0, fame = 0, poses = 0, banked = 0; const by = {};
...
console.log(`  FP/turn ${(fame / turns).toFixed(3)}  poses struck ${poses}  pose rounds PAID ${banked}`);
