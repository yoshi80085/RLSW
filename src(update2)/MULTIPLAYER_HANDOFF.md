...
- **3a — damage & fame math (pure, no wiring risk).** ☑ DONE. `marginToDamage`,
  ...
  `Game` math.
- **3b — rolls as actions.** `ATTACK_ROLLED { kind: swing|sonic|smash,
  attackerId, defenderId, mods }` → engine rolls via rng, stores
  `battle.rolls` + margin/damage verdict. The spin overlay reads the
  already-decided face; timing stays client. Bot + human paths share it.
...
| 3. Combat | ◑ **3a done** — pure combat math (`marginToDamage`, `fameFromMargin`, `knockbackSpaces`, `underdogBonus`) extracted to `engine/systems/combat.js`; `Game` imports all four (locals deleted; `underdogBonus` now takes the two Fame totals, keeping the spirit-identity guard in `Game`). selftest extended (bands, sonic caps, underdog ramp + exact regression grid vs old math) — full suite green; engine lint clean; HEAD+edits esbuild-compile clean. **3b–3e remain** (rolls→DAMAGE_APPLIED→KNOCKBACK_MOVED→KNOCKED_OUT→RETALIATION_* as actions; kills the `spiritsSynced` bridge). Owner: `npm run dev` smoke-test + commit from Windows. |
