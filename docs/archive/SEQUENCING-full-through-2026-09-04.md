<!-- ARCHIVED 2026-09-04. This is the COMPLETE SEQUENCING.md as it stood, all
     4763 lines: 31 session handoffs plus the original §0-§2 ordering thesis that
     had become buried between them at line ~1347.

     Nothing here was edited. The live src/SEQUENCING.md now carries the current
     handoff, the durable findings, and a dated index pointing back into this file.
     Search this file by section id (## 5-glow, ## 5-seats, ...). -->
...
<!-- APPENDED 2026-09-04b. Retired from src/SEQUENCING.md §A when
     handoff `5-cut` took the live slot. Unedited. -->

## 5-flags. session handoff, 2026-09-02i (the four dead flags)
...
<!-- APPENDED 2026-09-04c. Retired from src/SEQUENCING.md §A when
     handoff `5-hop` took the live slot. Unedited. -->

## 5-cut. session handoff, 2026-09-04b (the deletion)

🎯 **WA NO KOE IS GONE FROM THE CODE, AND THE CUT IS WHAT HAS A TEST NOW.** This
is `RONIN_ABILITY_DESIGN.md` §8.1 step **(a)** — the one item in the respec that
was blocked by nothing, and the one that had to go first because every other step
touches files the ability lived in. ⚠️ §B5 is this exact ability, this exact
file: the 2026-08-26 Shamisen cut ran past its own end, took `startNewTurnNotes`
with it, and left eighteen suites green. **So this pass was run in five steps
with `check:bundle` between each and `test:client` at the seam.**

### 5-cut.A What came out, and it was every item §2.4.1 predicted

`UPGRADE_SHOP_DESIGN.md` §3.3 measured the footprint two days ago and it was
accurate to the line. **Kernel:** `checkWaNoKoe` (the pure rule), the commit-time
application block, the `waNoKoe` seat on the commit report, and `waNoKoeBuffs`
from `economy.js`'s initial note state. **Client:** the `SIGNATURE_TESTS` row,
the commit flash, the turn-start `tickWaNoKoe()` call, the unlock log, and
`tickWaNoKoe` itself. **Data:** the skill row and its `desc` in `skillTree.js`,
and the Ronin's fourth `skillOrder` entry in `policies/bot.js`.

📌 **The Ronin's ladder is THREE deep** until 🌀 Shukuchi is built, and the **12 Db
mastery slot is empty**.

### 5-cut.B ⛔ THE DECISION IN THE PASS: THE ASSERTIONS WERE INVERTED, NOT DELETED

The three suites that stood on the ability (`melodyCommitCheck`, `shamisenCheck`,
`harnessCheck`) could have simply lost their rows. **That is the §B5 shape
exactly** — a deletion with nothing standing on it, where the next session re-adds
a `waNoKoeBuffs` seat "for compatibility" and every suite stays green.

✅ So `melodyCommitCheck` §13 is now the **REVIVAL GUARD**, the same move §14 made
for the delegation rewire and §5-flags made for the palette read:

- the rule is absent from `melodyCommit.js` **and** the seat from `economy.js`,
  read from source, so a re-add fails whatever it is called;
- the commit **report** carries no `waNoKoe` key and the **patch** no
  `waNoKoe`-shaped key, with the dead skill id deliberately unlocked — the half
  that survives a rename;
- 🎯 **and the number is the assertion.** A melody sitting entirely inside the
  Ronin's own stack (`C-E-G` on `C-E-G`) was a guaranteed fire worth +1 Drive.
  It now pays **0**. A `1` there is the ability back, from any file.
- `shamisenCheck` asserts `ABILITY_CD['wa_no_koe']` is `undefined` — **the
  cooldown table is the one door a dead id can walk back through** without
  anybody adding a skill row.
- §14 gained a third guard: no client `tickWaNoKoe`, because a tick with no
  writer walks an always-empty list forever and looks like it works.

### 5-cut.C 🐛 A BUG CLOSED BY DELETION, AND A TRAP THAT SURVIVED IT

`BOT_STRATEGY_HANDOFF.md` §7 has carried an open one-place fix for weeks: the
kernel **deliberately reproduced** the shipped bug where pre-commit `tempDrive`
was read off the render-scoped sheet and written back over the boost the same
commit had just earned — *a 5-note diatonic run worth +3 Drive ended at +1, on
exactly the turn he played best.* ✅ **The bug, the deliberate reproduction and
the pin all went with the rule.** §7 is closed and has been told, rather than
left waiting for a fix with nothing to fix.

⚠️ **THE SHAPE IS NOT CLOSED, ONLY THE INSTANCE.** Read-a-render-scoped-sheet,
write-back-over-a-fresh-patch was the **second** bug of this form in one function
(B10's `driveStack ?? sustainStack` was the first). The comment at
`rlsw-simulator-v3_8_1.jsx` ~7959 — where Bushido reads the LIVE sheet
specifically to avoid it — was rewritten to keep the warning without naming a
symbol that no longer exists. 📌 **A third would make it a class, and a class
wants a lint, not a comment.**

🔴 **AND ONE LIVE BUG WAS RESCUED FROM THE TOMBSTONE.** `GAME_BRIEF.md`'s Wa no
Koe section held a note that was never about Wa no Koe: `clearBattleBuffs` zeroes
the attacker's `tempDrive` and the defender's `tempSustain` after **every**
battle, so any "for N rounds" Drive buff dies the moment he swings — while the
counter keeps running and on expiry shaves a point off an unrelated later bonus.
**It applies to every temp buff in the game, it appears in no design doc, and
deleting the section around it would have deleted it.** It is now filed as its
own line.

### 5-cut.D 🚨 THE LEDGER IS FILED — four subtractions, still unanswered

§2.4.2 warned that three consecutive passes had each made the Ronin weaker and
deferred the compensation to the next, and that **the next pass must not make it
four.** It is four: the deletion landed. ✅ It is now a dated table in
`CHARACTER_HANDOFF.md` under **"THE RONIN LEDGER"**, with the question in one
line — *is a three-ability Spirit whose kit costs more and cools slower the
intended shape of a burst character, or is he owed a slot?*

🧊 ⚠️ **This is NOT a rebalance proposal and §B10 stands.** It is filed as a
decision waiting for its owner, which is the one thing three previous passes each
assumed a later one would do. **It is Alex's call, and it is not a session's.**

### 5-cut.E ✅ Verification

`check:bundle` **zero warnings** at every step (baseline, after the kernel, after
the client, after the data, after the suites). `test:client` green at the seam —
6 checks across 47 `.jsx` files. `test:melody` **165 → 163**: §13 lost 8
assertions (6 unconditional + 2 behind an `if`) and gained 5, §14 gained 1.
`test:shamisen` **34**, unmoved — one row removed, two better ones added.
`test:harness` **1559**, identical to §5-flags.F's figure, so the seeded stream
did not move. `test:arch` green after the `melodyCommit.js` exports row was
corrected — 📌 **it was also stale on line count, 693 against a real 707**, which
is the drift `test:arch` cannot see and `ARCHITECTURE.md` now records honestly.

⚠️ **`test:trace` DROPPED, 1205 → 1196, AND IT WAS CHASED DOWN RATHER THAN
WAVED THROUGH.** Two single-variable controls, on today's code:

| control | trace |
|---|---:|
| `bot.js` ladder alone put back to four | **1196** — not it |
| the `skillTree.js` row alone put back | **1205** |

🎯 **So the whole −9 is the skill row, and it is the §5-fans.D mechanism again,
not a lost check.** `botTraceCheck`'s counters are per-journal-entry: the loops
walk what the bot actually did, so one fewer purchasable skill on the Ronin
changes the match's shape and therefore how many entries there are to assert on.
**The assertions that matter did not move** — traced-equals-untraced and
audited-equals-plain are single equalities and both still hold, action for
action. 📌 **A count that falls because the game got smaller is not the same as a
count that falls because a check went away**, and the only way to tell them apart
is the control run. §B4 is the reason that distinction is worth 90 seconds.

### 5-cut.F 🎯 NEXT

1. 🌀 **BUILD SHUKUCHI — step (b), and it is the next build.** ✅ §2.5.2 **#1 is
   answered**: *he jumps over everything* — units, hazards, walls, slime (Alex,
   2026-09-04), knowingly a hard counter to area denial, with the brake being #2
   rather than a hazard exception. ⛔ **#2 is still open** and it is load-bearing:
   must all three hops be taken, and in one direction?
2. 🚨 **The Ronin ledger** (5-cut.D) — filed, waiting, Alex's.
3. 💰 **The flat unlock number**, then the Db sink. Unchanged in priority.
4. ⛔ **The client still won't colour the three endings** — `5-flags.G` item 1,
   untouched here and still true. It is VISUAL, so it wants a `.scratch/` preview
   first.
5. 💡 **Alex's new Wa no Koe form is in `IDEAS_INBOX.md`, undesigned on purpose.**
   ⚠️ If it is ever promoted it comes back as a **new rule with a new name and
   new coverage** — not by reviving the symbols this pass just guarded.


---

<!-- ARCHIVED 2026-09-04d, rotated out of src/SEQUENCING.md §A.
     Unedited. Indexed in that file's §C as `5-hop`. -->

## 5-hop. 🧭 START HERE — session handoff, 2026-09-04c (Shukuchi, built headless)

🎯 **THE RONIN HAS A FOURTH ABILITY AGAIN, AND ALEX'S ONE-LINE ANSWER CHANGED
WHAT IT IS.** This is `RONIN_ABILITY_DESIGN.md` §8.1 step **(b)**, taken the same
day as (a). ⛔ **The client half is deliberately NOT done** — the hop is a board
animation and a target overlay, and `CLAUDE.md`'s standing rule sends a visual to
a `.scratch/` preview before the monolith. `BOT_CLIENT_GAPS` names the gap.

### 5-hop.A ⭐ THE ANSWER THAT REWROTE THE ABILITY

§2.5.2 asked six questions and called #1 and #2 load-bearing. Alex answered #1
(*jumps over everything*) and then answered #4 with something that was not on the
ballot: **each hop costs 1 AP.**

That single sentence turned Shukuchi from a *movement turn* — spend the turn, get
six hexes, priced against nothing — into a **movement MODE**: while it is up, a
step buys two hexes and ignores what is between, billed from the same
`moveStepsLeft` pool as walking, the Swing and Bushido. **Three of §2.5.2's six
questions collapse into it:** #3 (does one hop eat the turn?) *no*; #4 (can he
act after?) *yes, if he can afford to* — no special rule, the pool answers; and
#2's worry evaporates because the brake is now a bill.

🎯 **AND THE BILL IS A BETTER BRAKE THAN THE GEOMETRY RULE §2.5.2 REACHED FOR.**
"Three free hops in any direction" was flagged as a 6-hex reposition *plus*
perfect facing on a burst character. At 1 AP a hop it is a trade against
everything else that turn: three hops is a Bushido he did not throw. A straight
line would have restricted every use equally; a price scales with what he wanted
to do instead.

### 5-hop.B What shipped

- ⭐ **`engine/systems/shukuchi.js`** (new, 146 lines) — the ring-2 landing set,
  the two-way gate, the hop reducer, the budget patch.
- **`data/gameConstants.js`** — five `SHUKUCHI_*` constants; **`cooldowns.js`** —
  a row in each of `ABILITY_CD` and `ABILITY_DB_COST`; **`economy.js`** — the
  `shukuchiHopsLeft` seat; **`turnFlow.js`** — its per-turn reset.
- **`actions.js` / `reduce.js`** — `SHUKUCHI_HOPPED`. ⚠️ It carries no cost
  argument: the AP is fixed inside the reducer, because a caller that can choose
  the price is a caller that can make the ability free.
- **`legalActions.js`** — emitted **beside `move`, not in the ability block**, and
  the placement is the rule. Shukuchi is not something he does *instead* of
  moving; it is what a move step buys while it is up. In the ability block the
  beam would price it against Bushido rather than against walking, and a line
  like `[hop, hop, swing]` would not be reachable.
- **`transition.js`** — a `shukuchi` case that pays through the same `firePatch`
  the client resolvers use, and picks up through the same `collectPickups` that
  walking onto a hex does.
- **`data/skillTree.js`** — the skill row, first in the Ronin's route.
- 🆕 **`shukuchiCheck.mjs`, `test:shukuchi`, wired into `test:all` in the same
  pass** (§B3), and its row in `ARCHITECTURE.md`.

### 5-hop.C ⛔ TWO DECISIONS INSIDE THE BUILD, AND THE FIRST IS A TRAP

**1. The second hop must not ask whether the ability is ready.** The first hop
starts a 3-round cooldown. A continuation that re-checked `canFire` would find
the clock it had just started and refuse — **Shukuchi would be a 2-hex blink
wearing a three-hop card, and every suite would have been green.** That is why
the budget lives on the sheet (`ns.shukuchiHopsLeft`) and `canHop` asks two
different questions depending on which hop this is. It is asserted directly:
*"hop 2 is legal WHILE THE COOLDOWN RUNS — the budget answers, not the clock"*.

📌 One counter, no companion "active" flag. Two fields can disagree about whether
the ability is running; one cannot.

**2. ⁉️ Per use or per hop? §2.5 said "1 Db per use" and was written when a use
was the whole movement turn.** Once hops are billed individually that sentence
has two readings a factor of three apart. ✅ **Built per-activation**, which is
what it literally says — so the Db and the clock are paid on the first hop and
hops two and three are free. ⚠️ **Which means the cooldown starts on the FIRST
hop**: a Ronin who hops once and thinks better of it has spent the ability.
**Per-hop is one constant away and it is Alex's call** (§2.5.0a).

⚠️ **AND THE HOP RE-FACES HIM, LIKE WALKING AND UNLIKE THE WARP.**
`applySpiritWarped` deliberately leaves facing alone. A hop that did the same
would let him land beside a rival still aimed down the lane he came from — the
free half of a Bushido setup, handed over for nothing.

### 5-hop.D 🐛 THE CANARIES BOTH FIRED, AND BOTH WERE RIGHT

Two suites went red on the first run and neither was a false alarm:

- **`legalActionsCheck` §16** — *"modelled kind 'shukuchi' is either driven by the
  client bot or named as a gap"*. Exactly its job: a rule shipped, the client bot
  had no path to it, and §5.A records that failing silently **eight times
  running**. ✅ `shukuchi` is now in `BOT_CLIENT_GAPS` with the reason and the
  consequence written out.
- **`selftest`** — *"a Spirit WITH a route still targets the head of it"* went red
  because the head moved. 📌 That is not cosmetic: the bot buys ~2.7 skills a
  match, so **the head of a route is bought almost always and the tail almost
  never.** Shukuchi is listed first deliberately — it is the cheapest thing in his
  kit and the only one worth something with no rival on the board.

### 5-hop.E ✅ Verification, and the counts were chased not waved through

`check:bundle` **zero warnings** at every step. All 24 suites green, run in
batches. 🆕 `shukuchi` **68**. `legal` 579 → **580** (§16 walks the kinds).
`transition` 256 → **257**. `skilltree` 129 → **135** (+6, the new row).
`melody` **163**, `shamisen` **34**, `riffparity` **127,598**, `client`, `render`
8/8, `b0`, `riff`, `arch` all green.

⚠️ **`harness` 1559 → 1537 and `trace` 1196 → 1205, and the cause is neither a
lost check nor the ladder order.** Two controls, one variable each:

| control | harness | trace |
|---|---:|---:|
| Shukuchi moved to the TAIL of the Ronin's ladder | **1537** | **1205** |
| the skill ROW removed from `skillTree.js` | **1559** | — |

🎯 **So the order does not move them at all and the row moves both.** It is the
§5-fans.D / §5-cut.E mechanism a third time: these suites count per-journal-entry
and per-seat, so one more purchasable ability changes what the bot buys, which
changes the shape of the match, which changes how many things there are to assert
on. 📌 **Three sessions running, a count has moved for this reason and each time
the only way to tell it from a regression was a single-variable control run.**
That is cheap and it is now the habit.

⚠️ **`test:all` does not finish inside a 2-minute shell on this VM**, and
`test:render` alone can exceed it cold because it esbuilds the monolith first —
it passes 8/8 when given room. `CLAUDE.md` now says so, because a timeout that
looks like a failure gets treated like one.

### 5-hop.F 🎯 NEXT

1. 🖥️ **SHUKUCHI'S CLIENT HALF — and it is a `.scratch/` preview first.** Until
   it lands the searcher plans hops the player cannot take, and any Ronin bench
   number reads a bot playing an ability the game will not let a human use.
   ⚠️ The preview wants levers for the things this build guessed at: the landing
   overlay, whether the arc is drawn over the hexes it clears, and what a spent
   hop budget looks like on the rail.
2. 🗡️👤 **Step (c) — respec Bushido and Shadow Illusion.** Pure number edits on
   shipped abilities. ⛔ Blocked on Alex confirming the flat +3 (§2.1.1).
3. ⁉️ **Per use or per hop** (§2.5.0a) — one constant, Alex's call.
4. 🚨 **The Ronin ledger is STILL open, and Shukuchi does not close it.** A 6 Db
   mobility tool where a 12 Db payout used to be; the 12 Db slot is empty.
   `CHARACTER_HANDOFF.md` → "THE RONIN LEDGER" now has a fifth row saying so.
5. 💰 **The flat unlock number**, then the Db sink. Unchanged in priority.
6. ⛔ **The client still won't colour the three endings** — `5-flags.G` item 1,
   untouched for a third session and still true.
...
<!-- APPENDED 2026-09-04e. Retired from src/SEQUENCING.md §A when
     handoff `5-hopport` took the live slot. Unedited. -->

## 5-hopui. session handoff, 2026-09-04d (the decision board; Shukuchi's overlay, preview only)

🎯 **NO SOURCE FILE CHANGED THIS SESSION, AND THAT IS THE POINT.** The
step the map named was Shukuchi's client half; `CLAUDE.md`'s standing rule sends
a visual to a `.scratch/` preview before the monolith. What shipped is the
preview, a probe that proves it agrees with the engine, and **two decisions that
had been sitting open**.

### 5-hopui.A ⭐ THE DECISION BOARD, AND WHY IT CAME FIRST

Alex asked for every open call in the repo on one board before any code — not
just the ones blocking the session. **Nine were live**, and the exercise paid for
itself twice over:

- 🚩 **One "open question" was already dead.** `STANCE_SYSTEM_DESIGN.md`
  §4 asks whether Intergalactic 0's stance should be "Behind the Back". The
  entire Stance system is **cut** and that doc is doubly superseded. A question
  in a superseded doc reads exactly like a live one to a grep — §B1 wearing a new
  hat.
- 🚩 **Two of the nine were filed only as prose inside a design doc**
  (the flat unlock number's riders in `UPGRADE_SHOP_DESIGN.md` §0⃣.3; the Riff
  Rat stat-block question in `RIFF_RAT_DESIGN.md` §0). Neither is in
  `STATE_OF_PLAY.md` §4. **A decision nobody can enumerate is a decision nobody
  can make** — §B9's lesson at one remove.

📌 **The board is not a doc and was deliberately not filed as one.** It is
derivable from `STATE_OF_PLAY.md` §4 + §7 and the `⁉️` markers, and a
tenth doc that must be kept in sync is exactly the drift this repo pays for
(§B1, §B9). What it *did* earn is the two rows now in §4.

### 5-hopui.B ✅ TWO DECISIONS CLOSED

| ⁉️ was open | ✅ Alex, 2026-09-04 | where it now lives |
|---|---|---|
| Is Shukuchi's 1 Db **per use or per hop**? | **Per activation.** Hop 1 pays the Db and starts the clock; hops 2–3 are free. The AP bill is the only brake. | `RONIN_ABILITY_DESIGN.md` §2.5.0a, now closed · `STATE_OF_PLAY.md` §4 |
| How does a hop **feel in the client**? | **Per-hop targeting** — ring 2 lights, one click per hop, walking interleavable with no toggling. | §2.5.0b, new · `STATE_OF_PLAY.md` §4 |

🎯 **THE SECOND ONE WAS NOT ON ANY LIST BEFORE THIS SESSION.** The engine
emits `shukuchi` beside `move`, so it *permits* interleaving; whether the player
may was never asked, and a client pass would have answered it by accident.

⚠️ **Alex's question about it is worth recording, because the answer
is a real distinction:** *"is it really that heavy? the code for the first hop is
the same as the 2nd and 3rd, no?"* — **Yes.** The only branch between the three
is `hopIsActivation`; same reducer, same target set, same overlay. The cost of
per-hop targeting is three **clicks**, not three implementations. The objection
was about the hand, never about the code, and it should never have been phrased
so it could be read as the latter.

### 5-hopui.C What shipped

- ⭐ **`.scratch/shukuchi-hop-preview.html`** — the real 111-hex board, the
  real `shukuchiLandings`, the real 238px HUD column with **all four** of the
  Ronin's signature buttons in it, and the twelve rail states. Levers for every
  taste call the build guessed at: overlay colour source (4, each with its
  collision named), fill/stroke/width, landing marker, arc style and rise, trail
  length and fade, budget readout (4), and the two ⚠️ legibility toggles
  — the **Db pip** and the **FREE** mark on hops 2–3.
- 🆕 **`.scratch/shukuchiparity.mjs`** — **896 assertions**, page vs
  engine. See .D.

### 5-hopui.D 🚩 THE PROBE EXISTS BECAUSE §5-glow.C ALREADY BIT US

A preview that **re-implements** geometry is a preview of a different game, and
nobody can see a ring-2 error on a screenshot. So the page carries a delimited,
DOM-free **parity region** — the board constants, `buildHexMap`,
`pointyCorners`, `axialDist`, `facingAngle` and `shukuchiLandings`, transcribed
— and the probe slices that region straight out of the HTML, runs it, and
diffs it against the real modules: five constants, every hex's axial and pixel
centre, and the landing set from **all 111 hexes under three blocking patterns**.

⚠️ **THE THIRD PATTERN IS THE ONE THAT MATTERS.** It blocks every
ring-1 hex the hop passes over and asserts the landing set is **unchanged at 12**.
An empty-board comparison would pass even if the page had quietly made the hop
care about what it jumps over — which is the entire ability. ✅ 896/896.

📌 **IT IS A PROBE, NOT A SUITE, AND THAT IS DELIBERATE** —
`CLAUDE.md` reserves `.scratch/` for evidence that lives one session. §B3 says a
check without a script is not a suite; it does **not** say every check earns one.
There is nothing shipped for this to guard, because the preview is not the
product. ⚠️ **When the overlay is ported, the thing that earns a suite is
the SSR diff of the shipped component**, not this.

### 5-hopui.E ✅ Verification

`check:bundle` **zero warnings**. **All 24 suites green**, run in batches, and
**every count identical to §5-hop.E** — which is the expected result and was
checked rather than assumed, because no source file changed:
`shukuchi` 68 · `legal` 580 · `transition` 257 · `harness` 1537 ·
`trace` 1205 · `skilltree` 135 · `melody` 163 · `stackslots` 115 ·
`slime` 127 · `eleven` 38 · `score` 122 · `eval` 156 ·
`winconditions` 79 · `turnflow` 73 · `battleflow` 65 · `determinism` 20 ·
`shamisen` 34 · `riffparity` 127,598 · `riff` 70,970 · `render` 8/8 ·
`client` 6 · `arch` 8 · `engine`, `b0` green. 🆕 `shukuchiparity` **896**.

⚠️ **`test:render` timed out at 175s cold and passed 8/8 on the next run**,
exactly as `CLAUDE.md` warns. A timeout here is not a failure and must not be
reported as one.

### 5-hopui.G ✅ AND THE DIAL-IN CAME BACK THE SAME SESSION

Alex screenshotted the control panel before closing it, so the port spec exists:
**`RONIN_ABILITY_DESIGN.md` §2.5.0c**, in the canonical doc rather than in
`.scratch/`, because the page keeps no state and a fresh copy loads with defaults.

Two of his choices are worth naming, because both are **bets, not preferences**:

- ⚠️ **He took Ronin blue `#4488ff` for the overlay — the same blue Psycho
  Bushido tints its targets with.** Two of his four abilities now light the board
  identically. 📌 The escape hatch is the **landing marker**, which he set to
  *none* and is therefore still free; the fix, when it bites, is a marker and not
  a hue. Written into §2.5.0c so nobody re-derives it.
- ⚠️ **He switched the "FREE" label on hops 2–3 OFF.** That label was the
  guard against §2.5.0a's trap — the clock starts on hop **1**, so a Ronin who
  hops once and stops has spent the ability. 🎯 The bet is that the segmented
  budget bar teaches it on its own: three bars, one greying per hop, and no second
  Db pip appearing. **If playtesters hop once and stop, this is the first line to
  change, and it is one checkbox.**

📌 **One thing the preview did not settle and the port must:** he wants the Db
pip ON *and* the segmented bar, and the bar has no natural place beside the pip
the way the dot readout did. That is a placement call inside the port, not a new
question for Alex.

### 5-hopui.F 🎯 NEXT

1. 🖥️ **PORT THE OVERLAY — ✅ NO LONGER BLOCKED.** Alex's dial-in arrived
   the same session and is captured in `RONIN_ABILITY_DESIGN.md` §2.5.0c (see .G). ⚠️ **Verify the port, do not assume
   it**: React SSR (`esbuild --jsx=automatic` + `react-dom/server`) diffed against
   the preview at the same settings. Then `shukuchi` comes **out** of
   `BOT_CLIENT_GAPS` and `legalActionsCheck` §16 becomes the thing that says so.
2. 🗡️👤 **Step (c) — respec Bushido and Shadow Illusion.** Pure
   number edits. ⛔ Still blocked on Alex confirming the flat +3 (§2.1.1), which
   is board row 3 and was left there on purpose.
3. 🚨 **The Ronin ledger** — board row 4, five rows deep, still nobody's.
4. 💰 **The flat unlock number**, then the Db sink. Unchanged in priority.
...
<!-- Previous live handoff archived 2026-09-05, content preserved. -->
